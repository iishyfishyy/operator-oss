 ...
 * The services feature flag ships ON (mirror lib/features.ts: unset/empty env
 * keeps the default), but hostname routing additionally requires the explicit
 * ORCH_SERVICE_HOSTS opt-in — enabling the feature alone exposes nothing. */
export function serviceRoutingEnabled() {
  const v = process.env.ORCH_FEATURE_SERVICES;
  return (
    (v ? truthy(v) : true) &&
    serviceHostsEnabled() &&
