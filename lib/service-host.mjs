...
/**
 * Public service hostnames (<slug>--<appHost>) are a separate opt-in from the
 * managed-services feature itself: OFF unless ORCH_SERVICE_HOSTS is set truthy.
 * The services feature ships enabled by default, and enabling it must never
 * expose anything on a public hostname — an instance additionally needs this
 * env AND a PUBLIC_BASE_URL (plus wildcard DNS/TLS for *--<host>) before the
 * router (lib/service-router.mjs) claims any hostname.
 */
export function serviceHostsEnabled(env = process.env) {
  const v = env.ORCH_SERVICE_HOSTS;
